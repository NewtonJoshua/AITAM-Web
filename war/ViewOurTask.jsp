...
<tr><th>Title</th><th>Activity</th><th>ID</th><th>Assignee</th><th>Status</th><th>Priority</th><th>Assigned On</th><th>Activity Due On</th><th>Task Due On</tr>
</tr>
<%for (TaskBean task:l){ %>
...
<td><form method="post" action="Controller"><input type="date" value=<%=task.getDueDate() %> name="subDate" required pattern="[2][0][1][0-9][-][0-1][0-9][-][0-3][0-9]" min="2015-01-01" title="yyyy-mm-dd" onchange="visible('Sub<%=task.getTaskId() %>')"></input>
<input name="submit1" value="changeSubDate" type="hidden"></input>
<input style="display:none"  align="right" type="image" width="18px"   src="images/Refresh.png" id="Sub<%=task.getTaskId() %>"></input>
<input type="hidden" name="id" value="<%=task.getTaskId() %>"></input>
</form></td>
<td><form method="post" action="Controller"><input type="date" value=<%=task.getMainDate() %> required pattern="[2][0][1][0-9][-][0-1][0-9][-][0-3][0-9]" min="2015-01-01" title="yyyy-mm-dd" name="MainDate" onchange="visible('Main<%=task.getTaskId() %>')"></input>
<input name="submit" value="changeDate" type="hidden"></input>
<input style="display:none"  align="right" type="image" width="18px"   src="images/Refresh.png" id="Main<%=task.getTaskId() %>"></input>
<input type="hidden" name="id" value="<%=task.getTaskId() %>"></input>
</form></td>
</tr>
<%} %>
